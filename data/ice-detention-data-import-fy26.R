...
clean_names <- raw_headers %>%
  ...
  mutate(
    ...
    V1 = str_trim(str_remove_all(V1, "FY26")),
    V1 = str_remove_all(V1, ":"), # Remove colons if present
    V2 = str_trim(str_remove_all(V2, "FY26")),
...
facilities_data_clean <- facilities_data %>%
  mutate(
    ...
    scheduled_inspection = if ("inspections_last_inspection_end_date" %in% names(.)) {
      if_else(
        !str_detect(inspections_last_inspection_end_date, "^[0-9]+$"),
    ...
    across(
      .cols = -c(facility_average_length_of_stay_alos:adp_mandatory,
...
facilities_data_clean_aggregate <- facilities_data_clean %>%
  mutate(
...
facilities_wikitable <- facilities_data_clean_aggregate %>%
  mutate(city_state = paste0(facility_city, ", ", facility_state),
