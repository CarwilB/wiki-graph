...
municipalities **and** for Bolivia's 9 departments and 112 provinces using census
counts from three surveys: 2001, 2012, and 2024 (CPV 2024).
...
Before adding new statements, a small number of pre-existing P1082 claims on seven
municipality items must be removed. Those items were tagged during a prior
data-cleaning pass; their existing values are undated or unreferenced and would
...
pop_prov             <- readRDS("data/poblacion_provincia.rds")
pop_dept             <- readRDS("data/poblacion_departamento.rds")
```
...
Unlike municipalities, we can map provinces and departments directly via the INE
code (P14142), which is now present on Wikidata for these items.

- Departments: 2-digit INE code, matching `cod.dep` from the INE community
  classification file.
...
The CPV 2024 population extracts already include department/province names.
We add INE codes as follows:

- **Departments:** take the first two digits of `row_num`-independent codes by
  grouping on `departamento` and joining to Wikidata via department INE code.
  (In practice `pop_dept` already has one row per department.)

- **Provinces:** take the first four digits of the 6-digit municipality code.
  Because `pop_prov` does not contain a province code, we compute it from the
  department/province order using the INE code on Wikidata: we join on
  `(departamento, provincia)` after extracting a clean province label from WD.

Note: Wikidata province labels are not perfectly standardized (e.g. multiple
"Cercado" provinces). We therefore match primarily by INE code (P14142). Where
that is missing, we fall back to a conservative name+department join and report
unmatched rows.

```{r fn-normalize-ine-code}
# Ensure INE codes are treated as fixed-width strings for joining.
normalize_ine_code <- function(x, width) {
...
```{r prep-wd-ine-codes}
departments_wd <- departments_wd |>
  mutate(ine_code = normalize_ine_code(ine_code, 2))

provinces_wd <- provinces_wd |>
  mutate(ine_code = normalize_ine_code(ine_code, 4))
```
...
pop_dept_qs <- pop_dept |>
  ...
  left_join(wd_dept_name, by = "dept_name")
...
## Step 6: Generate P1082 QuickStatements (municipalities, provinces, departments)
